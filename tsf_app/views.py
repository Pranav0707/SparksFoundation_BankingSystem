...
def customers(request):
        ...
        for customer in customers:
            ...
            if customer.email==semail and amt< int(customer.available_balance) and amt>0 :
                ...
                try:
                    ...
                    customer_detail1 = CustomerDetail(id=customer.id, available_balance=available_balance, email=customer.email
                                                    , name=customer.name)
                    transaction_detail2 = TransactionDetail(name=j.name, email=j.email,
                                                debit_amount=0 ,credit_amount=amt , account_balance=available_balance2)
                    customer_detail2 = CustomerDetail(id=id, available_balance=available_balance2, email=j.email
                                                    , name=j.name)
                    customer_detail1.save()
                    transaction_detail1.save()
                    customer_detail2.save()
    ...
    trans = TransactionDetail.objects.all().order_by('-id')
    context={
        'customers':customers,
        'trans':trans
    }
